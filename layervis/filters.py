...
class FilterVis:
        ...
        if current_step % self.blur_freq == 0 and self.enable_blur:
            if image.shape[-1] == 3:
                image = tf.convert_to_tensor(
                    gaussian(image.numpy()[0, ...], sigma=self.blur_size)[
                        np.newaxis, ...
                    ]
                )
            else:
                image = tf.convert_to_tensor(
                    gaussian(image.numpy()[0, ..., 0], sigma=self.blur_size)[
    ...
    def postprocess_image(
        self, image: np.ndarray | tf.Tensor, image_crop: int
    ) -> np.ndarray:
        """
    ...
    def plot_all_patterns(
        ...
        layers_list = [
            layer for layer in self.model.layers if isinstance(layer, self.valid_layers)
        ]
