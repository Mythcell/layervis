...
def plot_image_atlas(images, embedding, nx, ny, max_image_dist, grid_pad = 0,
    labels = None, normalize_embedding = True, label_colormap = None,
    label_display = 'border', border_thickness = 0.5, overlay_alpha = 0.1,
    figscale = 1, dpi = 100):
    """
    Plots an atlas of an imageset subject to some embedding.

    Args:
        images (np.array): Dataset of images to plot. Note there must
            at least be nx * ny images.
        embedding (np.array): 2D array of embeddings to visualise, i.e. coordinates
            of each image in a reduced, 2D plane, such as returned by t-SNE, UMAP, etc.
        nx (int): Number of images to display in the x-direction
        ny (int): Number of images to display in the y-direction
        max_image_dist (float): An image is only plotted for a given point (x,y)
            if the closest point in the embedding is less than this distance.
            Note the distance metric is euclidean.
        grid_pad (float): Amount to pad/offset the minimum and maximum x and y values
            of each plot in the grid. Useful for adding whitespace. Default is 0.
        labels (np.array): List of integer image labels. Note labels are only displayed
            if this parameter is supplied. Default is None.
        normalize_embedding (bool): Whether to normalize the provided embedding to
            the range [0,1]. Default is True.
        label_colormap (plt.cm): A matplotlib colormap object to colour the labels.
        ...
        label_display (str): One of 'border' or 'overlay'. If set to 'border', labels
        ...
        border_thickness (float): Thickness of the borders. Has no effect if
            label_display is not set to 'border'. Default is 0.5.
        overlay_alpha (float): Transparency of the overlaid rectangle. Has no effect
            if label_display is not set to 'overlay'. Default is 0.2
        figscale (float): Figure scale multiplier. The figure size is nx*figscale
            by ny*figscale. Default is 1.
        dpi (float): Base resolution, passed to plt.figure. Default is 100.
    """
    # first normalize the embedding
    if normalize_embedding:
        embedding[:,0] = (embedding[:,0] - np.min(embedding[:,0]))/np.ptp(embedding[:,0])
        embedding[:,1] = (embedding[:,1] - np.min(embedding[:,1]))/np.ptp(embedding[:,1])
    ...
    xvals = np.linspace(np.min(embedding[:,0])-grid_pad,
        np.max(embedding[:,0])+grid_pad,nx)
    yvals = np.linspace(np.max(embedding[:,1])+grid_pad,
        np.min(embedding[:,1])-grid_pad,ny)
    ...
    for i in yvals:
        for j in xvals:
            ...
            closest = min(enumerate(embedding),key=lambda x: euclidean_dist(x[1],(j,i)))
            distance = euclidean_dist(closest[1],(j,i))
            if distance <= max_image_dist:
                # plt.imshow(images[closest[0]])
                ax.imshow(images[closest[0],...])
                if show_labels: # extra insurance
